...
- 2026-06-28 · F21 · **Egyenleg Flow — saját dátumtartomány**: a fix 2 hónapos előrejelzés helyett a felhasználó `startDate` és `endDate` dátummezőkkel szabadon megadhatja a tervezési időszakot. A `horizonDays` mostantól ezekből számolódik (`parseDate` segítségével). Az „Előrejelzési időtáv" Select dropdown helyett egy violet kártyában két dátuminput (Kezdő / Záró) jelenik meg. Mentéskor `start_date` + `end_date` kerül az `egyenleg_flow` táblába (migráció `20260628_002`), a `horizon_days` fallback-ként megmarad a régi sorokhoz.
